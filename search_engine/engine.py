...
def get_matching_ids(tokens, inverted_index, max_docs=1000):
    posting_lists = []
...
def search_query(file_path, vocab, tfidf_data, inverted_index, norm2, idf):
    ...
    matching_ids = get_matching_ids(tokens, inverted_index)
    ...
    #     if doc_id in tfidf_data:
    #         doc_vector = tfidf_data[doc_id]
    #         norm2 = math.sqrt(sum(v ** 2 for v in doc_vector.values()))
    #         norm2_dict[doc_id] = norm2
    #         score = cosine_similarity_sparse(query_vector, doc_vector, norm1, norm2)
    #         if score > 0:
    #             results.append((doc_id, score))
    # with open("norm2.pkl", "wb") as f:
    #     pickle.dump(norm2_dict, f)
